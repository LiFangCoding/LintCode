 ...
 *
 * <p>Example Given candidate set [10,1,6,7,2,1,5] and target 8,
 *
 * <p>A solution set is:
 *
 * <p>[ [1,7], [1,2,5], [2,6], [1,1,6] ] Notice All numbers (including target) will be positive
 * integers. Elements in a combination (a1, a2, … , ak) must be in non-descending order. (ie, a1 ≤
    ...
    private void search(int start, boolean selected, int[] input, int target, List<Integer> path) {
        if (target == 0) {
            ...
            if (start != 0 && input[start] == input[start - 1]) {
                if (!selected) {
                    // just ignore the same. Only one candidate this condition. will not have: not selected,
                    // selecgted.
